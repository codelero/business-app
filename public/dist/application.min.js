"use strict";var ApplicationConfiguration=function(){var applicationModuleName="mean",applicationModuleVendorDependencies=["ngResource","ngAnimate","ui.router","ui.bootstrap","ui.mask","toaster","angularFileUpload","angularCharts"],registerModule=function(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)};return{applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:applicationModuleVendorDependencies,registerModule:registerModule}}();
...
"use strict";ApplicationConfiguration.registerModule("charts");
...
"use strict";angular.module("charts").config(["$stateProvider",function($stateProvider){$stateProvider.state("charts",{url:"/charts",templateUrl:"modules/charts/views/charts.client.view.html",controller:"ChartsController",resolve:{productItems:["Products",function(Products){return Products.query(function(response){},function(response){404===response.status?alert("Error accessing resource: "+response.config.method+" "+response.config.url):alert(response.statusText)}).$promise}]}})}]);
"use strict";angular.module("charts").controller("ChartsController",["$scope","productItems","ProductsUtilities","$filter",function($scope,productItems,ProductsUtilities,$filter){var i;for($scope.title="Price Analytics",i=0;i<productItems.length;i++)productItems[i].marginPercent=ProductsUtilities.calculateMarginPercent(productItems[i].price,productItems[i].cost),productItems[i].marginAmount=ProductsUtilities.calculateMarginAmount(productItems[i].price,productItems[i].cost);var orderedProductsAmount=$filter("orderBy")(productItems,"marginAmount",!0),filteredProductsAmount=$filter("limitTo")(orderedProductsAmount,5),chartDataAmount=[];for(i=0;i<filteredProductsAmount.length;i++)chartDataAmount.push({x:filteredProductsAmount[i].productName,y:[filteredProductsAmount[i].cost,filteredProductsAmount[i].price,filteredProductsAmount[i].marginAmount]});$scope.dataAmount={series:["Cost","Price","Margin Amount"],data:chartDataAmount},$scope.configAmount={title:"Top $ Margin Products",tooltips:!0,labels:!1,mouseover:function(){},mouseout:function(){},click:function(){},legend:{display:!0,position:"right"}};var orderedProductsPercent=$filter("orderBy")(productItems,"marginPercent",!0),filteredProductsPercent=$filter("limitTo")(orderedProductsPercent,5),chartDataPercent=[];for(i=0;i<filteredProductsPercent.length;i++)chartDataPercent.push({x:filteredProductsPercent[i].productName,y:[filteredProductsPercent[i].marginPercent]});$scope.dataPercent={series:["Margin %"],data:chartDataPercent},$scope.configPercent={title:"Top % Margin Products",tooltips:!0,labels:!1,mouseover:function(){},mouseout:function(){},click:function(){},legend:{display:!0,position:"right"}}}]);
...
"use strict";angular.module("core").config(["$provide",function($provide){$provide.decorator("$exceptionHandler",["$delegate",function($delegate){return function(exception,cause){exception.message="Please contact the Help Desk! \n Message: "+exception.message,$delegate(exception,cause),alert(exception.message)}}])}]);
...
"use strict";angular.module("products").config(["$stateProvider",function($stateProvider){$stateProvider.state("products",{"abstract":!0,url:"/products",template:"<ui-view/>"}).state("products.list",{url:"",templateUrl:"modules/products/views/list-products.client.view.html"}).state("products.detail",{url:"/:productId",templateUrl:"modules/products/views/detail-products.client.view.html"}).state("products.edit",{url:"/edit/:productId","abstract":!0,templateUrl:"modules/products/views/edit-products.client.view.html",data:{roles:["user","admin"]}}).state("products.edit.info",{url:"/info",templateUrl:"modules/products/views/edit-info-products.client.view.html"}).state("products.edit.price",{url:"/price",templateUrl:"modules/products/views/edit-price-products.client.view.html"}).state("products.edit.tags",{url:"/tags",templateUrl:"modules/products/views/edit-tags-products.client.view.html"})}]);
"use strict";angular.module("products").controller("ProductsController",["$scope","Products","$stateParams","$state","toaster","ProductsUtilities","Authentication",function($scope,Products,$stateParams,$state,toaster,ProductsUtilities,Authentication){$scope.authentication=Authentication,$scope.edit={product:{price:""},newTags:"",datePicker:{open:function($event){$event.preventDefault(),$event.stopPropagation(),$scope.edit.datePicker.opened=!$scope.edit.datePicker.opened}},priceOption:"percent",marginPercent:function(){return ProductsUtilities.calculateMarginPercent($scope.edit.product.price,$scope.edit.product.cost)},calculatePrice:function(){var price=0;console.log($scope.edit.priceOption),"amount"===$scope.edit.priceOption?price=ProductsUtilities.calculatePriceFromMarkupAmount($scope.edit.product.cost,$scope.edit.markupAmount):(console.log($scope.edit.markupPercent),price=ProductsUtilities.calculatePriceFromMarkupPercent($scope.edit.product.cost,$scope.edit.markupPercent)),$scope.edit.product.price=price}},$scope.showImages=!1,$scope.toggleImages=function(){$scope.showImages=!$scope.showImages},$scope.find=function(){$scope.products=Products.query()},$scope.findOne=function(){"new"===$stateParams.productId?($scope.edit.pageTitle="Create New Product",console.log($scope.edit.product)):Products.get({productId:$stateParams.productId},function(data){$scope.product=data,$scope.edit.product=data,$scope.edit.pageTitle="Update "+$scope.edit.product.productName,$scope.marginPercent=ProductsUtilities.calculateMarginPercent($scope.product.price,$scope.product.cost),$scope.edit.product.releaseDate=$scope.edit.product.releaseDate?new Date($scope.edit.product.releaseDate):"",$scope.product.tags&&($scope.tagList=$scope.product.tags.toString())})},$scope.addTags=function(tags){tags?($scope.error="",tags=tags.split(","),$scope.edit.newTags="",angular.forEach(tags,function(tag,index){$scope.product.tags.indexOf(tag)>1&&tags.splice(index,1)}),$scope.product.tags=$scope.product.tags.concat(tags)):$scope.error="Please enter one or more tabs separated by a comma"},$scope.removeTag=function(tag){var index=$scope.product.tags.indexOf(tag);$scope.product.tags.splice(index,1)},$scope.save=function(isValid){var product=$scope.edit.product;if(console.log(product),console.log("is",isValid),product.hasOwnProperty("productCode")&&(product.productCode=product.productCode.toUpperCase()),isValid)if("new"===$stateParams.productId){var newProduct=new Products(product);newProduct.$save(function(data){toaster.pop("success","Save Successful")},function(errorResponse){$scope.error=errorResponse.data.message})}else product.$update(function(){toaster.pop("success","Update Successful")},function(errorResponse){$scope.error=errorResponse.data.message});else toaster.pop("error","Please ensure all fields are valid")},$scope.cancel=function(){$state.go("products.list")}}]);
"use strict";angular.module("products").factory("ProductsUtilities",[function(){function calculateMarginPercent(price,cost){var margin=0;return price&&cost&&(margin=100*(price-cost)/price),margin=Math.round(margin)}function calculateMarginAmount(price,cost){var margin=0;return price&&cost&&(margin=price-cost),margin}function calculatePriceFromMarkupPercent(cost,percent){var price=cost;return cost&&percent&&(price=cost+cost*percent/100,price=Math.round(100*price)/100),price}function calculatePriceFromMarkupAmount(cost,amount){var price=cost;return cost&&amount&&(price=cost+amount,price=Math.round(100*price)/100),price}return{calculateMarginPercent:calculateMarginPercent,calculateMarginAmount:calculateMarginAmount,calculatePriceFromMarkupPercent:calculatePriceFromMarkupPercent,calculatePriceFromMarkupAmount:calculatePriceFromMarkupAmount}}]);
"use strict";angular.module("products").factory("Products",["$resource",function($resource){return $resource("api/products/:productId",{productId:"@_productId"},{update:{method:"PUT"}})}]);
